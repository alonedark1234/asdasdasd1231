...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **hiçbir odada değilsin!**");
    ...
    if (!SearchString) return client.sendTime(message.channel, `**girin lütfen - **\`${GuildDB.prefix}play [song]\``);
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (!player) return client.sendTime(message.channel, "❌ | **Şu anda çalışan bir parça yok...**");

        if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Müzik bulunamadı - **" + SearchString);
      ...
      console.log(e);
      return client.sendTime(message.channel, "**Gönderiler bulunamadı - **" + SearchString);
    }
  },
  ...
  SlashCommand: {
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Odanızda müzik çalın",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!member.voice.channel) return client.sendTime(interaction, "❌ | **Bu komutu kullanmak için bir odada olmalısınız!**");
      if (guild.me.voice.channel && !guild.me.voice.channel.equals(member.voice.channel)) return client.sendTime(interaction, ":x: | **Bu komutu kullanmak için bir odada olmalısınız!**");
      let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
      if (!CheckNode || !CheckNode.connected) {
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        try {
          ...
          if (res.loadType === "LOAD_FAILED") {
            if (!player.queue.current) player.destroy();
            return client.sendError(interaction, `:x: | **Arama yapılırken bir hata oluştu**`);
          }
        } catch (err) {
          return client.sendError(interaction, `**Arama yapılırken bir hata oluştu:** ${err.message}`);
        }
        switch (res.loadType) {
          case "NO_MATCHES":
            if (!player.queue.current) player.destroy();
